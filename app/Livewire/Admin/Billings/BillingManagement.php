...
class BillingManagement extends Component
{
    ...
    public function get_users_billing()
    {
        $usersWithBilling = User::select("*", DB::raw("CONCAT(users.first_name,' ',COALESCE(users.last_name, '')) as full_name"))
            ...
            // $join->on('user_addresses.user_id', '=', 'users.id')->where('user_addresses.user_id', '=', $this->user->id);
                ...
                $builder->when($this->startDateDeadline, function ($builder) {
                    ...
                    //$builder->where(function ($builder) {
                        if ($this->startDateDeadline && $this->endDateDeadline) {
                            ...
                            //$builder->whereBetween('due_date', [Carbon::parse($this->startDateDeadline)->startOfDay(), Carbon::now()->endOfMonth()]);
                           $builder->whereBetween('due_date', [Carbon::parse($this->startDateDeadline)->startOfDay(), Carbon::now()->addMonths(3)]);
                        }
                   // });
                });
            })
