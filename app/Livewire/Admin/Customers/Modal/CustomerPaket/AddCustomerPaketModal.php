...
class AddCustomerPaketModal extends Component
{
    ...
    public function addCustomerPaket(AddAddressCustomerPaketRequest $addressRequest)
    {
        $this->resetErrorBag();
        ...
        $this->customerPaketService->addCustomerPaket($this->user, $this->input);
        $this->dispatch('refresh-customer-list');
        LivewireAlert::title(trans('customer.paket.alert.success'))
            ->text(trans('customer.paket.alert.add-customer-paket-successfully', ['customer' => $this->user->full_name]))
            ->position('top-end')
            ->toast()
            ->status('success')
            ->show();
