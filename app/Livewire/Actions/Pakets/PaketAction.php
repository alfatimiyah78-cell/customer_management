...
class PaketAction
{
    ...
    public function add_paket(array $input)
    {
        ...
        $response = $this->create_profile_on_mikrotik_isnot_exist($paket);
    ...
    public function store_paket(array $input)
    {
        return Paket::create([
     ...
     * @param  array<string, string>  $input
     */
    public function trash_paket(Paket $paket, array $input)
    {
        // return dd('trash');
        if ($input['selectedPaket'] != $paket->id) {
            if (count($paket->customer_pakets)) {
                foreach ($paket->customer_pakets as $customer_paket) {
                    ...
                    try {
                        ...
                        if ($internetService == 'ppp') {
                            $this->pppService->updateProfileSecret($mikrotik, $customer_paket->ppp_paket, $profileName);
                        } else if ($internetService == 'ip_static') {
                            //
                        }
                        $customer_paket->update([
            ...
            if (count($paket->customer_pakets)) {
                ...
                foreach ($paket->customer_pakets as $customer_paket) {
                    ...
                    try {
                        if ($internetService == 'ppp') {
                            $this->pppService->disableSecret($mikrotik, $customer_paket->customer_ppp_paket, 'true');
    ...
    {
        //$newProfile = $this->importProfile($profile);
