...
<div class="flex gap-2 justify-end text-xs font-semibold">
    ...
    @moneyIDR($billing->amount - $billing->discount + $billing->tax - $billing->payments()->whereNull('refund_status')->sum('amount'))
</div>
...
@if ($billing->payments->count())
    <div class="flex justify-end text-xs">
            <flux:subheading size="sm" class="text-blue-700 dark:text-blue-300">
                ...
                @moneyIDR($billing->payments->sum('amount'))
...
@if ($billing->payments()->whereNotNull('refund_status')->count())
    <div class="flex justify-end text-xs">
            <flux:subheading size="sm" class="text-red-500 dark:text-red-300">
                ...
                @moneyIDR($billing->payments()->whereNotNull('refund_status')->sum('amount'))
            </flux:subheading>
    </div>
@endif
@if ($billing->payments()->whereNull('refund_status')->count())
    <div class="flex justify-end text-xs">
            <flux:subheading size="sm" class="text-green-700 dark:text-green-300 font-semibold">
                {{ trans('billing.label.received_amount') }}

                @moneyIDR($billing->payments()->whereNull('refund_status')->sum('amount'))
