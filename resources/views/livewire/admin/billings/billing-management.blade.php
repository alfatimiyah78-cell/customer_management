                    ...
                    <x-tables.cell>
                        ...
                        <div class="flex pt-1 pb-1 gap-2">
                                ...
                                @if (collect($selectedInvoice)->count())
                                    @if($search_with_status === 'paid')
                                        <flux:tooltip content="{{ trans('billing.payment.tooltip-bulk-unpayment', ['count' => collect($selectedInvoice)->count()]) }}">
                                            <flux:button size="xs" variant="danger" style="cursor: pointer;" disabled>
                                                {{ trans('billing.button.bulk-unpayment', ['count' => collect($selectedInvoice)->count()]) }}
                                            </flux:button>
                                        </flux:tooltip>
                                    @else
                                        <flux:tooltip content="{{ trans('billing.payment.tooltip-bulk-payment', ['count' => collect($selectedInvoice)->count()]) }}">
                                            <flux:button size="xs" variant="success" style="cursor: pointer;"
                                                wire:click="bulkSelectedPayment">
                                                {{ trans('billing.button.bulk-payment', ['count' => collect($selectedInvoice)->count()]) }}
                                            </flux:button>
                                        </flux:tooltip>
                                    @endif
                                @endif
