    ...
    <x-layouts.paket.layout :heading="__('paket.pakets')" :subheading="__('Pakets description')">
        ...
        <div class="md:flex-row flex-col flex gap-2 justify-between mb-4">
            <div class="md:flex-row flex-col flex justify-start gap-2">
                ...
                <x-select wire:model.change="search_server" id="search_server" class="mt-1 mr-1 pr-4">
                    <option value="" selected>
                        {{ trans('paket.ph.search-with-server') }}
                    </option>
                ...
                <x-select wire:model.change="search_with_status" id="search_with_status" class="mt-1 mr-1 pr-4">
                    <option value="">
                        {{ trans('paket.ph.search-with-status') }}
                    </option>
                    <option value="true">{{ trans('paket.status.disable') }}</option>
                    <option value="false">{{ trans('paket.status.enable') }}</option>
                </x-select>
            </div>
            <div class="md:flex justify-between gap-2">
                <flux:button wire:click="$dispatch('show-add-paket-modal')" style="cursor: pointer;" variant="primary"
                    icon="plus-circle">
                        ...
                        <x-tables.row>
                            ...
                            <x-tables.cell>
                                <div class="inline-flex">
                                    @if (!$paket->customer_pakets->count())
